class HomeController < ApplicationController
  def index
    ...
    @bolt_img = Product.where(:category_id=> bolt_cid).order('RAND()')
    @nut_img = Product.where(:category_id=> nut_cid).order('RAND()')
    @stamping_img = Product.where(:category_id=> stamping_cid).order('RAND()')
